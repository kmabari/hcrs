...
export default function JanamailAdminReport(){
 ...
 return <Card className="border-2 border-indigo-200 bg-white"><CardContent className="p-4 space-y-3">
  ...
  {rows.length>0&&<div className="overflow-x-auto"><table className="w-full min-w-[1200px] text-xs text-left text-slate-950"><thead className="bg-slate-200"><tr>{['#','Time','Name','Mobile','District','Place / Post','Subject','Email ID','Submission','Mail Launch','Record ID'].map(x=><th key={x} className="p-2 font-black">{x}</th>)}</tr></thead><tbody>{rows.map((r,i)=><tr key={r.id} className="border-t align-top"><td className="p-2">{i+1}</td><td className="p-2 whitespace-nowrap">{timeText(r)}</td><td className="p-2 font-black">{r.fullName||'-'}</td><td className="p-2 font-bold">{r.mobileNumber||'-'}</td><td className="p-2">{r.district||'-'}</td><td className="p-2">{r.placePost||'-'}</td><td className="p-2 max-w-72 whitespace-normal">{r.selectedSubject||'-'}</td><td className="p-2 break-all">{r.emailId||'-'}</td><td className="p-2"><Badge className="bg-emerald-100 text-emerald-950">{r.submissionStatus||'-'}</Badge></td><td className="p-2">{r.emailLaunchStatus||'-'}</td><td className="p-2 font-mono break-all">{r.id}</td></tr>)}</tbody></table></div>}
 </CardContent></Card>;
}
