...
async function startServer() {
  ...
  function generateServerFallbackResponse(userQuery: string, member: any, orgSettings?: any): string {
    ...
    if (member) {
      return `ഹലോ ${member.name}! വഴികാട്ടി AI താൽക്കാലികമായി കാര്യങ്ങൾ തനിപ്പകർപ്പാക്കുകയാണ്.\n\nതാങ്കളുടെ പ്രൊഫൈൽ വിവരങ്ങൾ താഴെയുണ്ട്:\n- പേര്: **${member.name}**\n- മെമ്പർഷിപ്പ് ഐഡി: **${member.membershipId || 'പെൻഡിംഗ് (Pending Verification)'}**\n- മൊബൈൽ നമ്പർ: **${member.mobile || member.phone || 'N/A'}**\n- എക്സ്പെയറി തീയതി (Expiry Date): **${member.expiryDate || member.renewalDate || '31-Dec-2026'}**\n- സ്റ്റാറ്റസ്: **${member.status || 'Active'}**\n\nനിങ്ങളുടെ അക്കൗണ്ടിലോ മൊബൈൽ നമ്പറിലോ കൂടുതൽ വിവരങ്ങൾ തിരുത്തണമെങ്കിൽ നേരിട്ട് താഴത്തെ ഹെൽപ്പ് ബട്ടണിലൂടെ ഓൺലൈൻ സപ്പോർട്ട് ടിക്കറ്റ് ആഡ് ചെയ്യാവുന്നതാണ്! 😊`;
    }
