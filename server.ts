    ...
    try {
      ...
      if (!sheetId) {
        ...
        return res.status(400).json({
          error: "Google Sheet ID is not configured on the server. Please set GOOGLE_SHEET_ID in your environment variables.",
      ...
      return res.status(500).json({
        error: err.message || "An error occurred while saving participant details to Google Sheets.",
